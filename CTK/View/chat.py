import os
from tkinter import scrolledtext, Scrollbar, messagebox
...
class ChatInterface:
    ...
    def chatting(self,conversation_id,recive_id,chatting):
        ...
        button_file = customtkinter.CTkButton(
            ...
            text="Choose a file",
            font=BUTTON_FONT,
            command=lambda: attach_file(conversation_id, recive_id)
        )
        button_file.grid(row=1, column=0, sticky='nsew', padx=5)
        ...
        if messages:
            ...
            for i, msg in enumerate(messages, start=1):
                ...
                print(msg['sender_id'])
                if msg['sender_id'] == int(Session.user_id):
                    ...
                    username_label.grid(row=0, column=0, sticky='e', padx=10, pady=5)
                if self.chatting1.is_file_message(msg['message_id']):
                    # Tạo label cho link tải file
                    file=self.chatting1.is_file_message(msg['message_id'])
                    file_link_label = ctk.CTkLabel(frame_message_a, text="Download File", fg_color="lightgreen",
                                                   cursor="hand2")
                    file_link_label.grid(row=1, column=1, sticky='w', padx=10, pady=5)

                    # Hàm callback để xử lý sự kiện nhấn vào link
                    print(msg['message_text'])
                    message_text = msg['message_text'].strip()
                    file_link_label.bind("<Button-1>",
                                         lambda e, file_path=file[0]['file_path']: self.chatting1.download_file(message_text))
        ...
        def attach_file( conversation_id, recive_id):
            ...
            # Gửi file qua socket
            try:
                self.chatting1.upload_file(file_path)
                self.chatclient.send_file(recive_id, conversation_id, Session.user_id, file_name, self.chatting1)
                messagebox.showinfo("File","Gui File thanh cong")
