...
from CTK.module import chatting


class ChatClient:
    HOST = '192.168.104.131'
    PORT = 1234
    ...
    def add_message(self, username, message, user_id,file_message=False):
        ...
        if user_id == Session.user_id:
            ...
            if file_message==True:
                ...
                file_label = ctk.CTkLabel(frame_message, text=message, fg_color="lightblue", wraplength=200)
                file_label.grid(row=0, column=1, sticky='w', padx=10, pady=5)

                # Nút tải file
                download_button = ctk.CTkButton(frame_message, text="Download",
                                            command=lambda: self.chatinterface.download_file(message))
                download_button.grid(row=0, column=2, sticky='e', padx=10, pady=5)
            username_label.grid(row=0, column=0, sticky='e', padx=10, pady=5)
            message_label.grid(row=0, column=1, sticky='w', padx=10, pady=5)
    ...
    def send_file(self, recive_id,conversation_id, sender_id, file_name, chatting):
        chatting.send_file(conversation_id, sender_id, "192.168.1.8/server_token/FileClient",file_name)
        try:
            self.add_message(Session.username,f"Sent a file: {file_name}",sender_id,file_message=True)
            # Gửi header trước
            header = f"FILE~{recive_id}~{sender_id}~{Session.username}~{file_name}"
                ...
                if message.startswith("SERVER~"):
                    ...
                    if content=="typing":
                     ...
                     self.chatinterface.typ.grid_remove()
                        ...
                        self.add_message(username,content,user_id,file_message=True)
                    else:
